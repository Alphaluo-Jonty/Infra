...
inline auto get_name(std::string_view name) {
  size_t colon_pos = name.find("::");
  if (colon_pos != std::string_view::npos) {
     return name.substr(colon_pos + 2);
  }
  return name;
}
...
inline auto get_conflict_key() {
  auto it = g_ormpp_conflict_key_map.find(get_name(ylt::reflection::type_string<T>()));
  if (it == g_ormpp_conflict_key_map.end()) {
    auto auto_key =
        g_ormpp_auto_key_map.find(get_name(ylt::reflection::type_string<T>()));
